...
rem Released builds are v<series>.<build number>, where the series comes from the
rem VERSION file and the build number from the CI run. There is no build number
rem here, so a local build says dev and carries the commit instead.
set SERIES=0.1
if exist VERSION set /p SERIES=<VERSION
set SHA=unknown
for /f "delims=" %%v in ('git rev-parse --short HEAD 2^>nul') do set SHA=%%v
set VERSION=v%SERIES%.0-dev+%SHA%

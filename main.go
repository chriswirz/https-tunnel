...
func run() error {
	...
	if runServer {
		srv, err := server.New(cfg.Server, logger.With("role", "server"), frontend(logger), cfg.SaveAdminPasswordHash)
		if err != nil {
			return err
		}
		wg.Add(1)
...
func clientOptions(cfg *config.Config, logger *slog.Logger) tunnelclient.Options {
	...
	if c.ServesDirectory() {
		...
		if c.LocalDir != "" {
			// Both are configured, which is allowed; say which one is in use rather than leaving it to be guessed from behaviour.
...
func (f *webFrontend) ServePage(w http.ResponseWriter, _ *http.Request, name string, status int) bool {
...
func (f *webFrontend) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case strings.HasPrefix(clean, "sessions/"):
		if f.ServePage(w, r, "sessions/__placeholder__", http.StatusOK) {
			return
		}
	}

...
Binaries for Linux, Windows and macOS are attached to every release at <https://github.com/chriswirz/https-tunnel/releases>. `main` carries a rolling prerelease, so the newest build is always there.
...
```bash
# 1. Fetch the binary (use -linux-arm64 on a Pi or a Graviton box)
curl -fsSL -o https-tunnel \
  https://github.com/chriswirz/https-tunnel/releases/latest/download/https-tunnel-linux-amd64
sudo install -m 0755 https-tunnel /usr/local/bin/https-tunnel
...
go tc.Run(ctx) // reconnects on its own until ctx is cancelled
```
...
Two complete programs live in `examples/`:
...
Both are compiled by `./build.sh` and `build.cmd`, and by CI on every platform, so a change to the library API cannot quietly break them. The release attaches them next to the main binary. Nothing is written to stdout by the library itself, so a program that speaks a protocol on stdout stays clean.
...
Afterwards, sign in with the chosen password. To reset a forgotten one, clear `password_hash` in the config file and restart: the server is back to first boot and accepts admin/admin once. Browser sessions live in memory, so a restart signs everyone out.
...
| `admin` | web UI account: `username`, `password_hash` (written by the server), `session_hours` |
...
| `trust_forwarded_headers` | honour `X-Forwarded-*` from a trusted front proxy |
...
The application icon comes from `appicon.png` (1024x1024). After replacing it, run `go run ./tools/mkicon` to regenerate `favicon.ico` (16, 32 and 48 px, served by the Go server at `/favicon.ico`) and `web/app/icon.png` plus `web/app/apple-icon.png`, which Next links from every page. `appicon.ico` is separate: it is the Windows executable icon.
...
examples/                embedded-mcp and embedded-dir use the library; docker/ holds compose stacks
...
- The administrator password is stored only as a PBKDF2-HMAC-SHA256 hash. A first boot session, admitted with admin/admin, can do nothing except set a real password.

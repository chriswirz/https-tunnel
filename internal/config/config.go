...
type ClientConfig struct {
	...
	// InsecureSkipVerify disables TLS verification towards the local target.
...
type ServerConfig struct {
	...
	// TrustForwardedHeaders honours X-Forwarded-For/Proto from a front proxy.
...
// SaveAdminPasswordHash writes a new administrator password hash into the server section of the config file.
// Only that one field is rewritten, so the rest of the document survives untouched.
func (c *Config) SaveAdminPasswordHash(hash string) error {

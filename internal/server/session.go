...
// requested is the label the client asked for, and it is honoured when it is free, or when it is held by another session of the same API key, which is that key reclaiming its own name after losing its session id.
...
// honoured reports which of the two happened, so the caller can say so in the log.
func (m *Manager) Create(keyName, requested string) (s *Session, honoured bool, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// ReapExpired removes disconnected sessions idle beyond the TTL.

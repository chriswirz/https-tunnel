...
type Frontend interface {
	...
	// vars replaces literal placeholders in the page before it is written, which is how a static export learns something only the server knows, such as which hostname the control plane is on.
	...
	ServePage(w http.ResponseWriter, r *http.Request, name string, status int, vars map[string]string) bool
}
...
type Server struct {
	...
	// saveAdmin persists a changed username or password hash, normally into config.json.
	// An empty string for either means "leave that one alone".
...
// saveAdmin persists an account change made in the UI, and may be nil in a build that has nowhere to write it.
func New(cfg *config.ServerConfig, logger *slog.Logger, ui Frontend, saveAdmin func(username, passwordHash string) error) (*Server, error) {
	...
	if cfg.Admin.PasswordHash == "" {
		// Naming the account matters: after a rename the default password belongs to the new name, and the old one is gone for good.
		logger.Warn("no admin password set, the web ui accepts this account with the password \"admin\" once and will demand a new one",
			"username", s.adminUsername(), "config", "server.admin.password_hash")
	}
	return s, nil
}
...
func (apiOnlyFrontend) ServePage(http.ResponseWriter, *http.Request, string, int, map[string]string) bool {
	return false
}
...
func (s *Server) controlPlane() http.Handler {
	...
	mux.HandleFunc("DELETE /api/v1/sessions", s.handlePruneSessions)
...
type connectRequest struct {
	...
	// SubdomainRequest is the label the client would like, gonored when it is free.
...
func (s *Server) handleConnect(w http.ResponseWriter, r *http.Request) {
	...
	sess, gonored, err := s.sessions.Create(key.Name, requested)
	if err != nil {
		writeJSONError(w, http.StatusServiceUnavailable, err.Error())
		return
	}
	if requested != "" && !gonored {
...
// pruneAges are the idle windows the UI offers, and the only ones this endpoint accepts.
// A fixed list keeps an accidental "1" or "30" from deleting far more than intended, and every one of them parses with time.ParseDuration except the last three, which are spelled out.
...
// handlePruneSessions deletes every disconnected session idle longer than the given age.
func (s *Server) handlePruneSessions(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		writeJSONError(w, http.StatusBadRequest,
			"idle_for must be one of "+strings.Join(sortedPruneAges(), ", "))
		return
	}

	removed := s.sessions.PruneIdle(idle)
	if removed == nil {
	...
	for _, v := range removed {
		labels = append(labels, v.Subdomain)
	}
	if len(removed) > 0 {
		s.logger.Info("pruned idle sessions", "idle_for", age, "count", len(removed), "subdomains", strings.Join(labels, ","))
	}
...
//
// To anyone not signed in, an offline tunnel is indistinguishable from a hostname that was never issued: both answer with the 404 page.
// Saying "tunnel offline" would confirm that a subdomain is real and merely idle, which is exactly what someone guessing labels wants to learn, and the label is the only thing protecting whatever the client serves when it comes back.
// A signed in administrator gets the page that explains what is wrong, since they can already read the whole session table.
func (s *Server) renderOffline(w http.ResponseWriter, r *http.Request, sess *Session) {
	admin, ok := s.authSession(r)
	...
	if s.ui.ServePage(w, r, "offline", http.StatusBadGateway, s.pageVars(r)) {
		return
	}
...
func (s *Server) renderNotFound(w http.ResponseWriter, r *http.Request) {
	...
	if s.ui.ServePage(w, r, "404", http.StatusNotFound, s.pageVars(r)) {
		return
	}
	http.NotFound(w, r)
}
...
// pageVars are the placeholders substituted into an exported page.
// A page served on a tunnel hostname cannot work out where the control plane lives: dropping the first label of km7z6jsfvjjd.t.chriswirz.com happens to give the right answer, and dropping it from t.chriswirz.com does not, and the page has no way to tell those apart.
// So the server, which does know, says.
func (s *Server) pageVars(r *http.Request) map[string]string {
	return map[string]string{
		"__TUNNEL_BASE_URL__": fmt.Sprintf("%s://%s", s.scheme(r), s.cfg.BaseDomain),
	}
}

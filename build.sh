...
# Colour only when a terminal is attached, so CI logs stay clean.
...
JSON
  ...
  curl -s -b "$smoke/jar" -X POST \
    -d '{"current_password":"admin","new_password":"a-longer-secret"}' \
    http://127.0.0.1:18080/api/v1/auth/password; echo

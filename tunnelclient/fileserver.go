...
// So when a file's bytes look like text, the extension's type is honoured only if a browser would actually render it, and anything else becomes text/plain.
...
//
// The list is short on purpose. "text/" is not a good enough test: a Debian box maps .md to text/markdown and .ts to text/vnd.trolltech.linguist, and every browser downloads both, which is the behaviour this whole path exists to avoid.
// Anything outside this set that reads as text is relabelled text/plain, so a readable file opens in the browser whatever the local mime table happens to call it.

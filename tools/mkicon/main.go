...
func writeICO(path string, master image.Image, sizes ...int) error {
	...
	for i, data := range images {
		...
		out.WriteByte(side)
		out.WriteByte(side)
		out.WriteByte(0)                                    // palette colors, 0 for truecolor
...
func resize(src image.Image, size int) *image.NRGBA {
	...
	for y := range size {
		...
		for x := range size {
			...
			// Alpha weighted, so transparent pixels do not drag color into the edges.
...
// unpremultiply divides a summed color channel by its summed alpha, giving the plain 8 bit value.
